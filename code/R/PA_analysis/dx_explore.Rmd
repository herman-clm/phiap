...
dx$VISIT_NUMBER_NUM <- as.character(dx$VISIT_NUMBER_NUM)
...
length(unique(temp$EMPI))
summary(dx)

```
There are only 843 distinct EMPI's in HERMANDA_ALDO_DX.csv
There are 8075 distinct EMPI's in HERMANDA_RAR_DX.csv
...
```{r}
first_dx <- dx %>% filter(EMPI %in% dx_empi$EMPI) %>%
  group_by(EMPI) %>%
...
```{r}
temp <- fread(file = "/data/raw_data/PA/HERMANDA_RAR_MEDS.csv", stringsAsFactors = FALSE, header = TRUE)
