...
rar_mg %<>% mutate(AVS_indicator = ifelse(!is.na(`Test_Left_Aldos_ng/dL`) | !is.na(`Test_Right_Aldos_ng/dL`) | !is.na(`Test_IVC_Aldosterone_ng/dL`), TRUE, FALSE), 
                   UAldo_indicator = ifelse(!is.na(`Test_URINE_ALDOSTERONE_ug/day`), TRUE, FALSE))
```
...
lab_epic %>% 
  filter(EMPI %in% empi) %>%
  filter(grepl("IVC A", RESULT_ITEM_CODE)) %>%
  group_by(EMPI, year(ENC_DATE))
  View() # 47
