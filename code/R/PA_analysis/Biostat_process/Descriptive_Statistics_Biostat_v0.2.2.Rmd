---
title: "Descriptive Statistics on v0.2.2 Bios Data"
output: 
  html_document:
    theme: journal
    toc: true
...
empi_grp10 <- pts %>% filter(PA_AVS_tot_0115 & is.na(Aldo) & is.na(PRA) & is.na(DRC)) %>%
              select(EMPI, PA_AVS_tot_0115, Dx_h0_Hyperaldo_n, Dx_h0_HTN_n, time_in_sys_yr, time_to_1st_hyperaldo_dx_yr)
...
tmp <- rar_mg %>% 
  group_by(EMPI) %>%
  summarise(RAR = any(!is.na(Aldo) | !is.na(PRA) | !is.na(DRC)), 
            PA_Dx = any(Dx_h1_Primary_aldosteronism, na.rm=T),
            RDB = any(PA_AVS_tot_0115, na.rm=TRUE)) %>% ungroup()
tmp
sum(rowSums(tmp[,2:4], na.rm = T) > 0)
