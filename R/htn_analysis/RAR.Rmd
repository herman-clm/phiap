...
tmp1 <- tmp[, c("Val.x", "CALCIUM", "CARBON_DIOXIDE", "CHLORIDE", "POTASSIUM",
                "spiro", "BP_DIASTOLIC", "Age", "GENDER_MASTER_CODE", "ALDOSTERONE")] %>%
  na.omit
...
aldo1 <- aldo1 %>%
  ...
  ungroup() %>%
    dplyr::select(EMPI, GENDER_MASTER_CODE, Age, ORDER_NAME.x, ORDER_START_DATE.x, ORDER_ITEM_CODE.x, ORDER_ITEM_DESCRIPTION.x, Val.x, BP_DIASTOLIC, BP_SYSTOLIC,
                   K, Loop, BB, CCBv, Thiazide, spiro, ACE_ARB, RESULT_ITEM_CODE.y, Val.y) %>%
...
set.seed(1)
tmp1 <- aldo1[, c("Age", "GENDER_MASTER_CODE", "aldo", "SODIUM", "POTASSIUM", "CHLORIDE", "GLUCOSE", "CREATININE", "UREA_NITROGEN", "CARBON_DIOXIDE", "CALCIUM",
                  "spiro", "K", "BP_SYSTOLIC", "BP_DIASTOLIC")] %>% na.omit
#tmp <- aldo1 %>%
